...
function CreateUserAgent() {
    ...
    // Use ws:// for localhost, wss:// for remote servers
    var wssServerLower = (wssServer || "").toLowerCase().trim();
    // More robust localhost detection
    var isLocalhost = wssServerLower === "localhost" || 
                      wssServerLower === "127.0.0.1" || 
                      wssServerLower.startsWith("localhost:") || 
                      wssServerLower.startsWith("127.0.0.1:") ||
                      (wssServerLower.includes("localhost") && !wssServerLower.includes(".")) ||
                      (wssServerLower.includes("127.0.0.1"));
    var wsProtocol = isLocalhost ? "ws" : "wss";
    // Use ServerPath if explicitly set, otherwise use empty string for direct Asterisk WebSocket connection
