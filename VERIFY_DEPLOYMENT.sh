...
echo "=== DSAI Deployment Verification ==="
echo ""
...
if grep -q "relations.*numbers" "dsai-app/backend/dist/agents/agents.service.js" 2>/dev/null; then
...
config_count=$(grep -c "\[CONFIG\]" "dsai-sts-deepgram/index.js" 2>/dev/null)
...
if docker ps --format "{{.Names}}" | grep -q "dsai-sts"; then
    container_name=$(docker ps --format "{{.Names}}" | grep "dsai-sts" | head -1)
...
if docker inspect dsai-asterisk --format '{{range .Mounts}}{{if eq .Destination "/var/spool/asterisk/monitor"}}MOUNTED{{end}}{{end}}' | grep -q "MOUNTED"; then
    echo "✅ Asterisk recordings volume mounted"
else
    echo "❌ Asterisk recordings NOT mounted - check docker-compose.yml"
fi

if grep -q "ASTERISK_MONITOR_PATH" "dsai-app/backend/.env" 2>/dev/null; then
