...
export class DockerService {
  ...
  async runContainer(
    ...
    // Also connect to 'dsai' network so backend can reach the container
    try {
      const dsaiNetwork = this.docker.getNetwork('dsai');
      await dsaiNetwork.connect({ Container: container.id });
      this.logger.debug(`Connected container ${name} to dsai network`);
    } catch (error: any) {
      this.logger.warn(`Failed to connect ${name} to dsai network: ${error.message}`);
    }
