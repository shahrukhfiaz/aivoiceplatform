...
export class AgentsService {
  private readonly defaultImage =
    process.env.CORE_DEFAULT_IMAGE || 'agentvoiceresponse/dsai-core:latest';
  ...
  private async initiateCallViaAgent(
    ...
    const context = `dsai-outbound-${agent.id}`;
    ...
    const response = await fetch(`${amiUrl}/originate`, {
      ...
      body: JSON.stringify({
        ...
        context,
        priority: 1,
        callerid: params.fromNumber ? `"${agent.name}" <${params.fromNumber}>` : `"${agent.name}" <dsai>`,
        // Pass variables to the dialplan
        variable: {
          DSAI_UUID: params.uuid,
          DSAI_TO_NUMBER: params.toNumber,
          DSAI_AGENT_ID: agent.id,
        },
