@echo off
REM AVR Cloud Server Deployment Batch Script
REM Server: 192.241.179.25
REM Username: root
REM Password: Seahub123@

echo ========================================
echo AVR Cloud Server Deployment
echo ========================================
echo.
...
if %ERRORLEVEL% NEQ 0 (
    ...
    echo.
    echo Or specify the full path to plink.exe in this script
    pause
    exit /b 1
)
...
plink.exe -ssh -pw Seahub123@ root@192.241.179.25 "docker --version && docker-compose --version && echo '' && echo 'Repositories:' && ls -la /opt/avr"
...
echo.
echo Next steps:
echo 1. SSH into server: ssh root@192.241.179.25
echo 2. cd /opt/avr/avr-infra
echo 3. Copy .env.example to .env: cp .env.example .env
echo 4. Edit .env file with your API keys
echo 5. Start services: docker-compose -f docker-compose-deepgram.yml up -d
echo.
pause
exit /b 0
